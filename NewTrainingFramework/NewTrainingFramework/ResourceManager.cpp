...
bool ResourceManager::LoadFromFile(const std::string& filepath) {
    // Debug: Try to get working directory (simple method)
    char buffer[1000];
    ...
    if (!file.is_open()) {
        ...
        // Try to check if file exists by attempting to open for reading
    ...
    while (std::getline(file, line)) {
        ...
        if (currentSection == "Model") {
            // Format: ID 0
            //         FILE "model\model1.nfg"
            if (line.find("ID") == 0) {
                int id;
                sscanf(line.c_str(), "ID %d", &id);
                
                // Read next line for FILE
                if (std::getline(file, line) && line.find("FILE") == 0) {
                    std::string filepath;
                    // Extract filepath from quotes
...
bool ResourceManager::LoadModel(int id, const std::string& filepath) {
    ...
    for (const auto& modelData : m_models) {
        if (modelData.id == id) {
            std::cout << "⚠️ Model with ID " << id << " already exists!" << std::endl;
            return false;
        }
    }
...
bool ResourceManager::LoadTexture(int id, const std::string& filepath, const std::string& tiling) {
    ...
    for (const auto& textureData : m_textures) {
        if (textureData.id == id) {
            std::cout << "⚠️ Texture with ID " << id << " already exists!" << std::endl;
            return false;
        }
    }
...
bool ResourceManager::LoadShader(int id, const std::string& vsPath, const std::string& fsPath) {
    ...
    for (const auto& shaderData : m_shaders) {
        if (shaderData.id == id) {
            std::cout << "⚠️ Shader with ID " << id << " already exists!" << std::endl;
            return false;
        }
    }
...
void ResourceManager::ClearModels() {
    m_models.clear();
    std::cout << "🗑️ Cleared all models" << std::endl;
}

void ResourceManager::ClearTextures() {
    m_textures.clear();
    std::cout << "🗑️ Cleared all textures" << std::endl;
}

void ResourceManager::ClearShaders() {
    m_shaders.clear();
    std::cout << "🗑️ Cleared all shaders" << std::endl;
}
...
void ResourceManager::PrintLoadedResources() {
    ...
    std::cout << "🖼️ Textures: " << m_textures.size() << std::endl;
